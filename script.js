document.addEventListener('DOMContentLoaded', () => {
    ...
    function getPriceForQuantity(quantity, pricingTiers) {
        // The pricing tiers are now pre-sorted
        for (const tier of pricingTiers) {
            if (quantity >= tier.moq) return tier.price;
        }
        // Fallback for single-tier items or if something goes wrong
        return pricingTiers[pricingTiers.length - 1].price;
    }

    function renderProducts() {
        // Sort pricing tiers for all products once on load
    ...
    // --- ADVANCED PDF GENERATION (from previous step) ---
